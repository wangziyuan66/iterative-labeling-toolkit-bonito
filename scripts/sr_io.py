    ...
    def run(self):
        ...
        with CSVLogger(summary_file(), sep='\t') as summary:
            for read, ctc_data in self.iterator:
                ...
                if reject_counter(mapping.r_en/mapping.ctg_len < self.end_quantile, f'stop at {mapping.ctg_len*100:.2f}%'): continue
                if reject_counter(mapping.r_st/mapping.ctg_len > self.start_quantile, f'start at {mapping.ctg_len*100:.2f}%'): continue
                # if reject_counter('N' in refseq, 'N_in_sequence'): continue
                # if mapping.r_en/mapping.ctg_len > self.end_quantile: end_ids.append(len(lengths))
                # if mapping.r_st/mapping.ctg_len < self.start_quantile: start_ids.append(len(lengths))
